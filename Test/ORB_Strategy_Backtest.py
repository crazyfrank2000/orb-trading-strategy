"""
ORB (Opening Range Breakout) Strategy Backtest System
基于IBKR数据的ORB策略回测
根据论文 "Can Day Trading Really Be Profitable?" 实现
"""
...
class ORB_Strategy:
    ...
    def _calculate_backtest_results(self):
        ...
        annual_return = (self.current_capital / self.initial_capital) ** (365 / days_in_market) - 1 if days_in_market > 0 else 0
    ...
    def plot_equity_curve_comparison(self, save_path=None):
        ...
        start_price = self.data[self.data['Date'] >= start_date].iloc[0]['day_close'] if not self.data.empty else 0
        end_price = self.data[self.data['Date'] <= end_date].iloc[-1]['day_close'] if not self.data.empty else 0
        
        # 计算买入持有的收益曲线
        hold_shares = self.initial_capital / start_price if start_price > 0 else 0
